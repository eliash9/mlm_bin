...
class Model_rekening extends CI_model{
    ...
    function rekening_tambah(){
        $datadb = array('nama_bank'=>$this->db->escape_str($this->input->post('a')),
                        'no_rekening'=>$this->db->escape_str($this->input->post('b')),
                        'pemilik_rekening'=>$this->db->escape_str($this->input->post('c')));
        $this->db->insert('rb_rekening',$datadb);
    }

    function rekening_edit($id){
        return $this->db->query("SELECT * FROM rb_rekening where id_rekening='$id'");
    }
    ...
    function rekening_update(){
        ...
        $this->db->update('rb_rekening',$datadb);
    }

    function rekening_delete($id){
        return $this->db->query("DELETE FROM rb_rekening where id_rekening='$id'");
    }
}