            <div class="col-xs-12">  
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Kirim Kode Distributor</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
        <?php
          $rows = $this->db->query("SELECT a.*, c.nama_distributor as nama_lengkap, c.telp_hp as no_hp, c.email, c.alamat_lengkap FROM rb_order_kode_konsumen a 
                                      JOIN rb_distributor c ON a.id_konsumen=c.id_distributor
            ...
            echo "<div class='col-md-12'>
                ...
                echo form_open_multipart('administrator/kirim_orderkodedistributor',$attributes); 
          ...
          echo "<table class='table table-condensed table-bordered'>
                  <tbody>
                    ...
                    <tr><th width='130px'><b>Jumlah PIN</b></th>              <td><select class='required form-control' name='a'>
                                                                                   ...
                                                                                   echo "</select></td></tr>
                    <tr><th><b>Pilih Paket</b></th>              <td><select class='required form-control' name='b'>
                                                                                      <option value=''>- Pilih -</option>";
                                                                                      $row = $this->model_members->paket();
                                                                                      foreach ($row->result_array() as $r){
                                                                                        if ($rows['id_paket']==$r['id_paket']){
                                                                                          echo "<option value='$r[id_paket]' selected>$r[nama_paket]</option>";
                                                                                        }else{
                ...
                <table class='table table-hover table-condensed'>
                  ...
                  echo "<tr>
                        ...
                        <td>$status</td>
                        <td><center>
                            <a class='btn btn-danger btn-xs' title='Delete Data' href='".base_url()."administrator/delete_orderkodedistributor/$row[id_order_kode_kirim]/".$this->uri->segment(3)."' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a>
                        </center></td>
                      </tr>";
                    $no++;
                  }
